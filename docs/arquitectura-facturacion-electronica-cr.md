...
| Reutilización de código | **Categoría A (se porta tal cual):** firma digital XML-DSig, generador XML v4.4, cliente OAuth de Hacienda, búsqueda CABYS. **Categoría B (se rediseña):** modelo de datos, consecutivos, validaciones pre-envío. Añadido en Fase 7: `ClaveNumericaGenerator` (clave numérica de 50 dígitos) también se portó tal cual como Categoría A, adaptado solo para aceptar el tipo de comprobante como `String` de 2 caracteres en vez del enum `TipoComprobanteElectronico` del proyecto original, que no existe en este esquema — ver el pendiente de compatibilidad Jakarta EE 11/Jackson 3 de la sección 10 para el resto de piezas de Categoría A todavía no portadas. |
...
- [x] Confirmar en el servicio de alta de producto que el campo `impuesto` de la respuesta de la API de CABYS no llegue vacío o nulo antes de permitir guardar el producto: **resuelto en Fase 6** — `ProductoService#validarYObtenerCabys` rechaza con `CabysSinImpuestoException` (400) si el campo llega nulo, sin defaultear a un porcentaje silencioso, mismo principio de `NOT NULL` que ya aplica a `codigo_cabys`.
...
- [ ] **Carrera entre validación de vigencia de exoneración y persistencia de la línea** (detectado en Fase 7): `FacturaService#aplicarExoneracion` valida en Java que la `cliente_exoneracion` esté vigente, pero esa lectura no toma un lock — si otra petición la desactiva (`ClienteExoneracionService#desactivar`) en la ventana entre esa validación y el `flush` de la línea, el trigger `fn_validar_exoneracion_vigente` (sección 4.15.2) la rechaza con un `RAISE EXCEPTION` que no se traduce a `DataIntegrityViolationException` (otra clase de SQLSTATE) — el cliente HTTP recibiría un 500 sin categorizar en ese caso, no el 400 limpio del resto del flujo. Ventana de carrera angosta y de bajo impacto práctico (requiere una desactivación concurrente exacta); documentado para no descubrirse por sorpresa, no bloquea Release 1.
- [ ] **Catálogo de tipos de exoneración excluidos duplicado, sin fuente única de verdad** (detectado en Fase 7): los 4 códigos exclusivos de Nota de Crédito/Débito (`01`, `05`, `06`, `07`, sección 4.15.1) están hardcodeados dos veces — `FacturaService.TIPOS_EXONERACION_EXCLUSIVOS_NC_ND` en Java y la lista literal `IN ('01','05','06','07')` dentro de `fn_validar_exoneracion_vigente` (sección 4.15.2) — sin ningún mecanismo que las mantenga sincronizadas. Hoy coinciden; una futura actualización de uno sin el otro divergiría en silencio, sin que ningún test lo detecte (ningún test inserta en `linea_factura` sin pasar por `FacturaService`). No bloquea Release 1.
