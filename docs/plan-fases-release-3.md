...
> **Cierre (2026-08-28):** implementada y mergeada a `main` vía 9 unidades de trabajo encadenadas (PRs [#25](https://github.com/EmaSleal/fractall/pull/25)-[#33](https://github.com/EmaSleal/fractall/pull/33), `stacked-to-main`) + PR de aterrizaje final [#34](https://github.com/EmaSleal/fractall/pull/34) (necesario porque los PRs intermedios se habían mergeado entre sí sin retargetear a `main`, mismo patrón que el cierre de Fase A). Commits de aterrizaje: `50a6ca4` (PR1) y `e7f5176` (PR2-PR6). Change SDD `invitacion-usuarios-cambio-tenant`, verificado con `sdd-verify` (PASS WITH WARNINGS, sin críticos) y archivado. Ver `sdd/invitacion-usuarios-cambio-tenant/design` (Engram) para el detalle de las tres discrepancias que el diseño encontró contra este mismo documento, corregidas en el texto de abajo.
...
## Fase C — Modelo de datos: `cobro_factura` — ✅ CERRADA

> **Cierre (2026-08-29):** implementada y mergeada a `main` vía 5 unidades de trabajo encadenadas (PRs [#35](https://github.com/EmaSleal/fractall/pull/35)-[#39](https://github.com/EmaSleal/fractall/pull/39), `stacked-to-main`) + PR de aterrizaje final [#40](https://github.com/EmaSleal/fractall/pull/40) (mismo patrón de aterrizaje que Fase A y Fase B). Change SDD `cobro-factura-modelo-datos`, verificado con `sdd-verify` (PASS limpio, 0 críticos, 0 warnings, 564/564 tests) y archivado. Dos decisiones no contempladas en la redacción original de esta fase, resueltas por el usuario durante `sdd-propose` y ya reflejadas en el texto de abajo: neteo del saldo/tope contra Notas de Crédito aceptadas (evita cobrar de más sobre una factura con NC parcial — mismo principio de "solución integral, no parche" ya aplicado en el cierre de Fase A), y bloqueo pesimista (`SELECT ... FOR UPDATE`) sobre la factura padre al registrar un cobro, para que dos cobros concurrentes no puedan sumar juntos por encima del saldo (mismo precedente que `contador_consecutivo`, sección 4.9).
...
- Trigger de tope de sobre-cobro (`BEFORE INSERT ON cobro_factura`): `SUM(monto_cobrado)` existente más el registro nuevo no puede exceder el saldo neto de la factura. **Decisión resuelta por el usuario (neteo contra NC):** el tope y la vista de saldo comparan contra `factura.total` **menos** el total de Notas de Crédito aceptadas que referencian esa factura (`tipo_comprobante='03'` + `comprobante_electronico.estado='ACEPTADO'`), no contra `factura.total` en bruto — entregarlo sin ese neteo habría sido un defecto conocido enviado a sabiendas, no alcance diferido. Un cobro que alcanza exactamente el saldo neto se permite (`>` estricto); el mínimo exceso lo bloquea el trigger. **Corrección post-exploración:** el trigger de tope de Nota de Crédito (Release 2, Fase A) en realidad vive sobre `comprobante_electronico` (`WHEN tipo_comprobante='03'`), no sobre la propia fila de NC, porque `tipo_comprobante` solo existe en `comprobante_electronico` y la NC se valida en el momento en que se crea ese envoltorio fiscal, no la orden comercial — un problema de orden de persistencia específico de ese flujo. `cobro_factura` no tiene esa restricción: cada fila es inequívoca en el momento en que se inserta, así que el trigger va directo sobre la propia tabla, sin la indirección de NC. Mismo principio de acumular-vs-tope, topología más simple.
...
- `POST /facturas/{id}/cobros`: registra un cobro parcial/total. Precondición validada en backend: `factura.condicion_venta IN ('02', '03', '04')` y comprobante en estado `ACEPTADO` (no tiene sentido registrar cobro sobre una factura que Hacienda todavía no aceptó o que fue rechazada). Reutiliza el patrón ya usado en `NotaCreditoDebitoService` para leer `comprobante_electronico.estado` por `factura_id` (nunca desde `factura`). Antes de validar y escribir, bloquea la fila de la factura (`SELECT ... FOR UPDATE`) para que dos cobros concurrentes no puedan sumar juntos por encima del saldo. Responde `201` con un cuerpo compuesto: la fila de cobro creada más la proyección de saldo recalculada, para que el frontend no necesite una segunda llamada. `registrado_por` viene siempre del principal autenticado, nunca del cuerpo de la solicitud.
- `GET /facturas/{id}/cobros`: historial de cobros de una factura específica, para reconciliación, más la proyección de saldo actual. Una factura fuera de alcance (`condicion_venta` distinto de `'02'/'03'/'04'`) responde distinto a una factura inexistente — "no elegible" (400) nunca se confunde con "no encontrada" (404).

**Criterio de salida:** extensión de `AislamientoMultiTenantTest` con los mismos ejes de cobertura ya exigidos en Release 2 Fase A: (a) un cobro no puede referenciar una factura de otro tenant, (b) el trigger de tope bloquea un cobro que excede el saldo neto (neteado contra NC aceptadas), (c) un segundo cobro parcial sobre la misma factura sí se permite mientras no exceda el saldo, (d) un intento de registrar cobro sobre una factura de contado (`condicion_venta = '01'`) es rechazado, (e) un intento de registrar cobro sobre una factura de arrendamiento (`condicion_venta = '05'` o `'06'`) es rechazado por la misma razón que contado. La vista `factura_estado_cobro` devuelve `estado_cobro` correcto para los tres casos (`PENDIENTE`/`PARCIAL`/`COBRADO`) contra datos de prueba que cubran `'02'`, `'03'` y `'04'`, no solo crédito. Dos cobros concurrentes que juntos exceden el saldo dejan exactamente uno aceptado, probado con hilos reales (no llamadas secuenciales disfrazadas de concurrentes). **Cumplido.**

---

## Fase D — Motor de reportes: IVA mensual y Flujo de Caja — ✅ CERRADA

> **Cierre (2026-08-30):** esta fase se partió en dos cambios SDD independientes durante `sdd-propose`, porque D.1 y D.2 no comparten query, agregado, invariante ni modo de falla — solo un prefijo de URL y un encabezado de este documento. **Change 1 — `reporte-iva-mensual`** (D.1 + D.3) está implementado, mergeado a `main` y archivado vía PRs [#41](https://github.com/EmaSleal/fractall/pull/41)-[#48](https://github.com/EmaSleal/fractall/pull/48) + aterrizaje final [#49](https://github.com/EmaSleal/fractall/pull/49). Verificado con `sdd-verify`: PASS (0 críticos, 0 warnings), 597/597 tests. **Change 2 — `reporte-flujo-caja`** (D.2) implementado, mergeado a `main` y archivado vía PRs [#50](https://github.com/EmaSleal/fractall/pull/50)-[#56](https://github.com/EmaSleal/fractall/pull/56) (7 unidades de trabajo, con PR #57 como retargeting de PR #51 hacia `main` directamente tras auto-cierre de GitHub). Commit de aterrizaje final: `812b3f4`. Verificado con `sdd-verify`: PASS (0 críticos, 0 warnings, 1 suggestion — cobertura de fixture en LEFT JOIN NULL-tolerant, defect-neutral por semántica SQL). Reuso de convención de D.3 confirmado: `CursorPdf.java` con zero-diff, 3 hojas `Resumen`/`DetalleVentas`/`DetalleCobros`, cardinality enforcement estructural. Hallazgo colateral no planeado durante D.2: defecto real ya en producción en `factura_estado_cobro` y trigger `fn_validar_alcance_cobro_factura` — Notas de Crédito/Débito heredan `condicion_venta` de su origen, pasando checks de eligibilidad sin confirmar que sean documentos cobrables (`tipo_comprobante IN ('01','04')`). Corregido en V24 (PR1 de D.2), mismo criterio de "solución integral, no parche" ya aplicado en Fase A. Tercer defecto detectado por análisis de diseño SDD en este proyecto (después de `cobro-factura-modelo-datos` y el `FacturaPdfService` inline-exoneración de D.1).
>
...
### D.2 — Reporte de Flujo de Caja — ✅ CERRADO (change SDD `reporte-flujo-caja`)

- `GET /reportes/flujo-caja?desde=&hasta=`: dos series temporales separadas, no una sola cifra — ventas del período (`comprobante_electronico.fecha_emision`, comprobante `ACEPTADO`) y cobros del período (`cobro_factura.fecha_cobro`). Un cobro de agosto puede corresponder a una factura de julio; mezclarlas en una sola serie produce un número que no responde ni "cuánto vendí" ni "cuánto cobré".
...
### D.3 — Exportación en dos niveles (convención general, no exclusiva de estos dos reportes) — ✅ CERRADO para IVA, reutilizado sin modificación por D.2
...
- **Excel:** misma separación pero como hojas, no páginas — pestaña `Resumen` (agregados) + N pestañas `Detalle*` (una fila por transacción del tipo correspondiente, sin agregación, cada tipo con sus propias columnas).
...
- Esta convención de "Resumen + Detalle(s)" queda establecida como estándar para cualquier reporte exportable futuro del sistema, no se rediseña reporte por reporte. **Confirmado reutilizado por D.2 sin modificación:** `CursorPdf.java` zero-diff, 3 hojas `Resumen`/`DetalleVentas`/`DetalleCobros` en ese orden, cardinality enforcement estructural (`DetalleVentas` 10 columnas sin `medioPago`, `DetalleCobros` 9 columnas con `medioPago`+`descripcionMedioPago`), header re-registration para N secciones de detail, PDF page-1 aggregates + page-2+ sections con per-section header repetition.
...
**Criterio de salida (D.2 — cumplido):** contra un set de datos de prueba con facturas a crédito/consignación/apartado con cobros parciales y una NC aceptada, el reporte de flujo de caja devuelve dos series separadas (ventas y cobros), con desglose correcto por condición/medio_pago respectivamente; la cartera pendiente a un corte pasado excluye cobros posteriores, NCs posteriores a ese corte, E invoices posteriores a ese corte (3 bounds independientes); la exportación reutiliza la convención de D.3 sin modificarla — hojas `Resumen`/`DetalleVentas`/`DetalleCobros` en ese orden, cardinality estructural, per-section headers en PDF.
