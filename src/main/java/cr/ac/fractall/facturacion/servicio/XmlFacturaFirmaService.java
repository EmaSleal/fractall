 ...
 * cifrado en {@code certificado_hacienda} (envelope encryption vía la KEK de Transit, Fase 5) y el
 * PIN vive en Vault KV (subruta {@code certificado/{ambiente}/pin}, ver {@code EmpresaService}).
 * {@link #firmar(String, UUID, String)} reemplaza esos dos parámetros de ruta+pin por una resolución
 * interna a partir de {@code empresaId} y {@code ambiente}: descifra el {@code .p12} y lee el PIN
 * antes de firmar -- ver el javadoc de la implementación para el detalle exacto de ese proceso.
 */
public interface XmlFacturaFirmaService {

    /**
     * Firma digitalmente {@code xml} con el certificado {@code .p12} de la empresa indicada para
     ...
     * @param ambiente ambiente de Hacienda ({@code SANDBOX} o {@code PRODUCCION}) del que se
     ...
     *     para ese ambiente, el PIN no está en Vault, el keystore/PIN no son válidos, o la firma
     *     en sí falla
     */
    String firmar(String xml, UUID empresaId, String ambiente);

    /**
     * Verifica criptográficamente la firma XAdES-BES de un XML ya firmado -- usado en pruebas para
     * demostrar que {@link #firmar(String, UUID, String)} produce una firma real y verificable;
     * ningún consumidor productivo la invoca todavía dentro del alcance de esta sub-tarea (ver el
     * javadoc de {@code ComprobanteXmlPersistenceService}).
     *
